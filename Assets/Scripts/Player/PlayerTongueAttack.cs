...
public class PlayerTongueAttack : MonoBehaviour
{
    ...
    IEnumerator TongueAttack(Vector3 direction)
    {
        ...
        foreach (Enemy enemy in enemies)
        {
            ...
            if ((enemy.gameObject.layer == layer + 6) &&
                Vector3.Dot((enemy.transform.position - transform.parent.position).normalized, direction) > 0.9f &&
                enmDistance < nearestDistance && enemy.enemyActive)
            {
